...
    getLoans().then((res) => {
      // Urutan dari ID terkecil ke terbesar
...
    const wscols = [{ wch: 5 }, { wch: 25 }, { wch: 30 }, { wch: 20 }, { wch: 20 }, { wch: 12 }, { wch: 12 }];
...
  // --- FUNGSI ZOOM QR CODE ---
  const handleZoomQR = (loanId) => {
    Swal.fire({
      title: 'Scan QR Peminjaman',
      html: `
        <div style="background: white; padding: 20px; display: inline-block; border-radius: 10px; border: 2px solid #DB2777">
          <img src="https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${loanId}" alt="QR Code" />
          <p style="margin-top: 10px; font-weight: bold; color: #4D2C3D">ID Pinjam: ${loanId}</p>
        </div>
      `,
...
          <p>📍 <b>LOKASI RAK:</b> <span style="color: #DB2777; font-size: 18px; font-weight: 800;">${loan.book?.Rak || '-'}</span></p>
        </div>
      `,
      icon: 'info',
...
            { id: "scan", label: "📸 Scanner", adminOnly: true, color: "#10B981" },
...
              <h2 style={{ textAlign: 'center', color: colors.deepPink }}>📷 Scan QR User Di Sini</h2>
              <Scanner />
            </div>
          )}
  ...
  <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 10px" }}>
    <thead>
      <tr style={{ color: colors.deepPink }}>
        <th>No</th><th>Judul</th><th>Peminjam</th><th>Aksi</th>
      </tr>
    </thead>
    <tbody>
      {loans.map((loan, index) => (
        <tr key={loan.id} style={{ background: colors.bgCream, borderRadius: "15px" }}>
          ...
          <td style={{ padding: "15px" }}>
            <b>{loan.book?.judul || "Buku Dihapus"}</b> <br/>
            {!loan.returned_at && (
              <div onClick={() => handleZoomQR(loan.id)} style={{ cursor: "pointer", marginTop: "10px", background: "white", padding: "5px", display: "inline-block", borderRadius: "5px", border: "1px solid #DB2777" }}>
                <QRCodeCanvas value={loan.id.toString()} size={70} />
                <p style={{ fontSize: "8px", margin: 0, color: colors.deepPink }}>🔍 Zoom QR</p>
              </div>
            )}
          </td>
          ...
          <td style={{ padding: "15px" }}>
            {!loan.returned_at ? (
              <button onClick={() => handleReturn(loan.id)} style={{ background: colors.deepPink, color: "white", border: "none", padding: "8px 15px", borderRadius: "10px", fontWeight: "bold" }}>
                {user?.role === "admin" ? "Selesai" : "↩️ Kembali"}
              </button>
